<script>
    ...
    async function loadAllAds() {
        ...
        if (user) {
            // Fazemos UMA única busca na tabela products trazendo os relacionamentos
            ...
            if (allAds) {
                // O Svelte separa a lista única em duas categorias para as abas
    ...
    async function handleDelete(id, type, e) {
        ...
        if (confirm('Tem certeza que deseja excluir este anúncio?')) {
            // O CASCADE no banco apaga a máquina automaticamente quando deletamos o produto
                        ...
                        <div class="relative overflow-hidden rounded-xl bg-white shadow-sm border border-gray-100 hover:shadow-md transition-all">
                            <div class="flex gap-4 p-4">
                                ...
                                <div class="flex flex-1 flex-col justify-center">
                                    ...
                                    <p class="mt-0.5 text-sm font-medium text-green-700">{formatPrice(maq.price)} <span class="text-xs font-normal text-gray-500">/hora</span></p>
                                    <div class="flex gap-2 mt-2">
                                        <span class="rounded-full bg-gray-100 px-2 py-1 text-[10px] font-semibold text-gray-600 uppercase tracking-wider">
                                            {maq.agricultural_machinery?.[0]?.machinery_types?.name || 'Máquina'}
                                        </span>
                        ...
                        <div class="relative overflow-hidden rounded-xl bg-white shadow-sm border border-gray-100 hover:shadow-md transition-all">
                            <div class="flex gap-4 p-4">
                                ...
                                <div class="flex flex-1 flex-col justify-center">
                                    ...
                                    <p class="mt-0.5 text-sm font-medium text-amber-700">{formatPrice(prod.price)}</p>
                                    <div class="flex gap-2 mt-2">
